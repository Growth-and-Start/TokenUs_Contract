...
const nftAbi = parseAbi([
  ...
  "function createAndMintOriginal(uint256,address,address,address,uint256) returns (uint256[])",
  "function createAndMintDerivative(uint256,uint256,address,uint16,address,uint256) returns (uint256[])",
...
// 원본 작품 생성 + nft 민팅 (통합)
export async function makeOriginalNFT(cfg: {
  rpcUrl: string;
  ...
  console.log("SDK function 'makeOriginalNFT' received cfg:", cfg);
  ...
  const { request } = await publicClient.simulateContract({
    address: cfg.nft,
    abi: nftAbi,
    functionName: "createAndMintOriginal",
    args: [
      cfg.workId,
      cfg.account,
      (cfg.split ?? "0x0000000000000000000000000000000000000000") as Address,
      cfg.account,
      amt,
    ],
    account: cfg.account,
...
// 파생 작품 생성 + nft 민팅 (통합)
export async function makeDerivativeNFT(cfg: {
  rpcUrl: string;
  chain: any;
  account: `0x${string}`;
  nft: Address;
  ...
  const { request } = await publicClient.simulateContract({
    address: cfg.nft,
    abi: nftAbi,
    functionName: "createAndMintDerivative",
    args: [
      cfg.parentTokenId,
      cfg.childWorkId,
      cfg.account,
...
export async function createOriginalWork(cfg: {
  ...
  const { request } = await publicClient.simulateContract({
    address: cfg.nft,
    abi: nftAbi,
    functionName: "createOriginalWork",
    args: [
      cfg.workId,
      cfg.creator,
      (cfg.split ?? "0x0000000000000000000000000000000000000000") as Address,
...
export async function mintEditionBatch(cfg: {
  ...
  const { request } = await publicClient.simulateContract({
    address: cfg.nft,
    abi: nftAbi,
    functionName: "mintEditionBatch",
    args: [cfg.to, cfg.workId, amt],
    account: cfg.account,
  });
  return walletClient.writeContract(request);
}
...
export async function mintDerivativeEditionBatch(cfg: {
  ...
  const { request } = await publicClient.simulateContract({
    address: cfg.nft,
    abi: nftAbi,
    functionName: "mintDerivativeEditionBatch",
    args: [cfg.to, cfg.childWorkId, cfg.parentTokenId, amt],
    account: cfg.account,
  });
  return walletClient.writeContract(request);
}
...
export async function getTokensOfWork(
  workId: bigint,
  chain: import("viem").Chain,
  nft: Address
) {
